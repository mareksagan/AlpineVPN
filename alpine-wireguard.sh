...
# A5: Anti-DPI: ARP filtering (ignore ARP from non-local networks)
...
# A3: Anti-DPI: DSCP/ToS field zeroing (don't mark traffic priority)
net.ipv4.tcp_ecn = 0
...
alpine_tune_interfaces() {
	...
	# Apply to WireGuard interface as well
	if ip link show wg0 >/dev/null 2>&1; then
...
randomize_mtu() {
	...
	if [ -f /etc/wireguard/wg0.conf ]; then
		sed_inplace "s/MTU = .*/MTU = $_rand_mtu/" /etc/wireguard/wg0.conf
...
while true; do
	...
	wget -q -O /dev/null --user-agent="$_user_agent" --timeout=10 "$url" 2>/dev/null || \
		curl -s -o /dev/null -A "$_user_agent" --max-time 10 "$url" 2>/dev/null || true
done
EOF
	...
	# Add to startup
	if [ "$os" = "alpine" ]; then
		echo '/usr/local/bin/chaff-generator.sh &' >> /etc/local.d/wireguard.start
	elif [ "$os" = "ubuntu" ] || [ "$os" = "debian" ]; then
	...
	if [ "$os" = "alpine" ]; then
		if apk add --no-cache simple-obfs 2>/dev/null || \
		   apk add --no-cache obfs4proxy 2>/dev/null; then
			...
			# Note: Requires additional configuration for use with WireGuard
